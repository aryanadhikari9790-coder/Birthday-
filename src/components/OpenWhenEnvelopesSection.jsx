  ...
  return (
    <section className="my-16 px-4 max-w-5xl mx-auto">
      <div className="text-center mb-12">
        <div className="inline-flex items-center space-x-2 bg-purple-500/10 border border-purple-500/30 px-4 py-1.5 rounded-full text-purple-300 text-xs font-semibold uppercase tracking-wider mb-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {letters.map((item) => (
          <div
            ...
            className="glass-card glass-card-hover p-6 rounded-3xl cursor-pointer group flex items-center space-x-4 border-rose-500/30 relative overflow-hidden"
          >
            {/* Envelope Icon */}
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-tr ${item.color} flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <h4 className="text-lg font-bold text-white group-hover:text-rose-300 transition-colors truncate">
                {item.trigger}
              </h4>
      ...
      {activeLetter && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="relative max-w-lg w-full glass-card p-6 md:p-8 rounded-3xl border-rose-500/50 shadow-2xl animate-scale-up">
            {/* Close Button */}
            <button
              onClick={handleClose}
            ...
            {/* Letter Content */}
            <div className="bg-slate-900/90 border border-slate-800 p-6 rounded-2xl text-slate-200 text-sm md:text-base leading-relaxed whitespace-pre-line font-sans max-h-96 overflow-y-auto">
              {activeLetter.letter}
            </div>

            {/* Modal Footer */}
            <div className="mt-6 flex justify-between items-center pt-2">
              <span className="text-xs text-rose-300 font-handwriting text-xl">
